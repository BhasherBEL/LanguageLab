...
<script lang="ts">
    ...
    import { AcademicCap, Sparkles, Icon, User as UserIcon, MagnifyingGlass } from 'svelte-hero-icons';
    ...
    onMount(async () => {
        ...
        (function (C: Window & { Cal?: any }, A: string, L: string) {
            let p = function (a: unknown, ar: unknown) {
                (a as any).q.push(ar);
            };
            let d = C.document;
            C.Cal =
        ...
        Cal('on', {
            ...
            callback: async (e: { detail: { data?: { date: string; duration: number } } }) => {
                ...
                {#each contacts as c (c.id)}
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="flex items-center text-lg">{c.nickname}</div>
                        </button>
                    </li>
                {/each}
            </div>
...
<dialog class="modal" open={modalNew}>
    <div class="modal-box">
        ...
        <form method="dialog" class="mt-4">
            <button class="btn float-end" on:click={() => (modalNew = false)}>
                Close
            </button>
        </form>
    </div>
