...
@usersRouter.get("/agents", response_model=list[schemas.User])
def get_agents(
    db: Session = Depends(get_db),
    current_user: schemas.User = Depends(get_jwt_user),
):
    ...
    try:
        agents = crud.get_agent_users(db)
        print(f"Found {len(agents)} agent users")
        return agents
...
@usersRouter.get("/{user_id}/llm-sessions", response_model=list[schemas.Session])
def get_user_llm_sessions(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: schemas.User = Depends(get_jwt_user),
):
    ...
    if (
        ...
        raise HTTPException(
            status_code=401,
            detail="You do not have permission to view this user's LLM sessions",
        )
    ...
    return crud.get_user_llm_sessions(db, user_id)
